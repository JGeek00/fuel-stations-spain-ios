...
struct SearchMunicipalitiesList: View {
    ...
    @EnvironmentObject private var municipalitiesViewModel: SearchMunicipalitiesViewModel
    ...
    var body: some View {
        NavigationStack(path: $navigationPath) {
            Group {
                if municipalitiesViewModel.loading == true {
                    ProgressView()
                        .transition(.opacity)
                }
                else if municipalitiesViewModel.data != nil {
                    let filtered = searchText != "" ? municipalitiesViewModel.data!.filter() { $0.Municipio!.lowercased().contains(searchText.lowercased()) } : municipalitiesViewModel.data!
                    Group {
            ...
            .navigationDestination(for: Municipality.self) { value in
                SearchStationsList()
                    .environmentObject(SearchStationsViewModel(selectedMunicipality: value))
            }
        }
    }
    ...
    private func DataList(data: [Municipality]) -> some View {
        ...
        case .groupedProvince:
            ...
            List(groupedByProvince, id: \.self) { item in
